...
Explica o código pedido a pedido. A aplicação agora também usa cookies HttpOnly,
Helmet, rate limiting e paginação — a secção 16 cobre essa camada.
...
│   ├── css/style.css
│   └── js/
│       ├── api.js         fetch() + cookie HttpOnly + paginação.
│       ├── app.js         Catálogo.
│       ├── login.js       Login.
...
sessão / cookie             JWT em cookie HttpOnly
...
3. `credentials: 'include'` manda o cookie HttpOnly (se existir).
   O JavaScript **não lê** o JWT. O browser envia-o sozinho.
   No catálogo público o cookie não é obrigatório — o GET é público.
...
O servidor responde com `Set-Cookie` (HttpOnly). O JS **não** guarda o JWT.
O cookie viaja sozinho nos pedidos seguintes por causa de `credentials: 'include'`.

### 7.2 Rota pública — `routes/authRoutes.js`

```js
router.post('/login', loginLimiter, authController.login); // sem JWT, com rate limit
router.post('/logout', authController.logout);
router.get('/me', authMiddleware, authController.me);      // com cookie ou Bearer
```

Login tem de ser público: ainda não tens sessão.
...
| JWT            | Token assinado; viaja em cookie HttpOnly (ou Bearer no Postman) |
| Cookie HttpOnly| Cookie que o JavaScript da página não consegue ler              |
| Helmet         | Middleware que mete cabecalhos HTTP de segurança (CSP, etc.)    |
| Rate limit     | Tecto de pedidos por IP / intervalo de tempo                    |
| Paginação      | `page` + `limit` + `totalPages` nas listagens                   |
...
## 16. Camada mais profissional (o que mudou)

Quatro peças novas. O percurso Front-end → API → Express → POO → MySQL
mantém-se. Estas peças metem-se **à volta**.

### Cookie HttpOnly — `utils/authCookie.js`

Antes o JWT ia no JSON e o JS guardava-o em `localStorage`. Qualquer script
na página (XSS) podia fazer `localStorage.getItem('autostand_token')` e
roubar a sessão.

Agora o login faz `res.cookie('autostand_token', jwt, { httpOnly: true, sameSite: 'lax' })`.
O JavaScript **não vê** o cookie. O `fetch` com `credentials: 'include'` envia-o.

`Secure: true` só em produção (`NODE_ENV=production`), porque em localhost
estás em HTTP e o browser recusaria o cookie.

O Postman continua a funcionar: se não houver cookie, o `authMiddleware`
aceita `Authorization: Bearer ...`.

`POST /api/auth/logout` apaga o cookie.

### Helmet — `app.js`

O Helmet mete cabecalhos como `X-Frame-Options` (dificulta meter o site
num iframe) e uma **CSP**: scripts só de `/js/...` (por isso o `onclick="logout()"`
saiu do HTML — scripts inline seriam bloqueados). O CSS do Bootstrap no CDN
está na lista branca.

### Rate limiting — `middleware/rateLimiters.js`

- API: 200 pedidos / 15 minutos por IP.
- Login: 10 falhas / 15 minutos. Um login **certo** não conta
  (`skipSuccessfulRequests`).

Quando passas do limite, a API responde 429.

### Paginação — `utils/pagination.js`

`GET /api/veiculos?page=2&limit=9` já não devolve a tabela inteira.

O repository faz `COUNT(*)` (total de linhas que batem com os filtros)
e `LIMIT x OFFSET y` (só a página pedida).
...
O catálogo e as tabelas do back-office desenham os botões Anterior / 1 2 3 / Seguinte.
